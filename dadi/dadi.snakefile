...
# models: dic of dic of dics, second level are model names, a dic with funcname: name of function specifying model; model_params: list of parameter names (MUST be pop sizes start with N, times with T, migration rates with m); upper_bound: list with parameter upper_bounds; lower_bound: list with parameter lower bounds
...
### NEED TO REFORMAT FIRST INPUT SO IT WILL BE TAKEN FROM INPUTSFS IN CONFIG AND THE WILDCARD IS PAIR INSTEAD OF P1 P2, SINCE WE HAVE DIFFERENT SFS TO COMPARE FORM SAME POP!!!!!

rule dadi_format_sfs:
    input:
        sfs = lambda wildcards: config["inputsfs"][wildcards.pair]
    output:
        sfs_unfolded = os.path.join(OUTMAIN, "2dsfs_dadi", "{pair}_unfolded_dadi.sfs"),
#        sfs_folded = os.path.join(OUTMAIN, "2dsfs_dadi", "{p1}_{p2}_folded_dadi.sfs")
    params:
        pop1 = lambda wildcards: config["pairs"][wildcards.pair][0],
        pop2 = lambda wildcards: config["pairs"][wildcards.pair][1],
        n1 = lambda wildcards: config["popn"][config["pairs"][wildcards.pair][0]],
        n2 = lambda wildcards: config["popn"][config["pairs"][wildcards.pair][1]]
    run:
...
rule plot_dadi_sfs:
    ...
    output:
        sfs_unfolded_plot = os.path.join(OUTMAIN, "2dsfs_dadi", "{pair}_unfolded_dadi.png"),
        sfs_folded_plot = os.path.join(OUTMAIN, "2dsfs_dadi", "{pair}_folded_dadi.png")
    params:
        pop1 = lambda wildcards: config["pairs"][wildcards.pair][0],
        pop2 = lambda wildcards: config["pairs"][wildcards.pair][1],
    run:
...
rule fit_model:
    input:
        dadisfs = os.path.join(OUTMAIN, "2dsfs_dadi", "{pair}_unfolded_dadi.sfs")
    output:
    ...
    run:
        ...
        #p0 = np.random.random(npars) * (upper - lower) + lower # draw initial guesses for params from unif(lower_bound, upper_bound)

        # use dadi pertrube params funciton to draw intitial guess. this will make them all closer to 1, might be better for convergnece? dont' know but is the recommended way in dadi manual
        p0 = np.ones(npars)
        np.random.seed(int(params.seed))
        p0 = dadi.Misc.perturb_params(p0, fold=2, upper_bound=upper, lower_bound=lower)
...
rule converge_getmaxlike:
    ...
    params:
        ...
        lower_bound = lambda wildcards: config["models"][wildcards.model]["lower_bound"],
    log:
        tmpout = os.path.join(OUTMAIN, "fit", "{pair}", "{model}", "temporal_collected_pars_allruns.txt"),
    run:
        ...
        while not converged:
            ...
            df.to_csv(log.tmpout, index=False, header=True, sep="\t", )
