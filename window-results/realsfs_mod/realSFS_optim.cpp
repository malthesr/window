/*
   A slightly modified version of the ANGSD original. Changes include:
   - Log counts, not frequencies
   - Log to stderr, not to stdout
   */
...
double emAccl(double *p,double tole,int maxIter,int nThreads,int dim,std::vector<Matrix<T> *> &gls,int useSq,int verbose){
    ...
    fprintf(stderr,"startlik=%f\n",oldLik);fflush(stderr);
    if (verbose) {
        fprintf(stderr, "Initial SFS:");

        for (int i=0;i<dim;i++) {
            fprintf(stderr, " %f", ((double)gls[0]->x)*p[i]);
        }
    ...
    while(SIG_COND&&iter<maxIter){
        ...
        if (verbose) {
            fprintf(stderr, "Iteration %d, current SFS:", iter + 1);
